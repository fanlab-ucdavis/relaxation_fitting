...
if isfield(RF,'G') && isfield(RF,'ref')
    ...
    %   Check that the correct number of RF waveforms are in the options structure
    %   If needed, read external waveform and scale to desired tip
    if isempty(RF.RF) || size(RF.RF,1) ~= length(RF.angle) || size(RF.RF,2) ~= Nrf
        if length(RF.path) ~= length(RF.angle)
            for i = 1:length(RF.angle)
                [~,RF.RF(i,:),RF.path{i}] = readRF('',Nrf,[str ' ' num2str(i)]);
            end
        else
            for i = 1:length(RF.angle)
                [~,RF.RF(i,:),RF.path{i}] = readRF(RF.path{i},Nrf,[str ' ' num2str(i)]);
            end
        end
%         RF = scaleRF(RF);
    end
    RF = scaleRF(RF);   %   Always scale waveforms to desired tip
