...
switch lower(opt.mode(1))
    ...
    case 's'
        ...
        opt.RFe = calcflipRF(opt.RFe,z,B1,0);
        Me = replicate*sin(opt.RFe.alpha);
        
        %   Create array of flip angles at each echo train [etl x length(alpha)]
        opt.RFr = calcflipRF(opt.RFr,z,B1,0);
        
        %   Call compiled EPG code to compute echo train amplitudes
        Mr = epgMEX(opt.T1(T2),T2,opt.esp,opt.RFr.alpha);
        ...
        %   Debug
%         if (any(isinf(S)) || any(isnan(S)) || ~isreal(S) || all(S==0))
%             warning('Strange signal');
%         end
